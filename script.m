...
DRAW = 1; % (Re-)draw ROIs? 1=YES. 0=NO.
FILENAMEcontainingNematicLines = 'example image containing nematic lines.tif';
...
if DRAW == 1
    ...
    figure(1); clf;
    RGB = imread(['input/' FILENAMEcontainingNematicLines]);
    IMG = rgb2lab(RGB);
    BWall = (IMG(:, :, 2) > 1);
    imshow(BWall);
    ...
    for i = 1:CC0.NumObjects
        ...
        y1 = y(Imin);
        y2 = y(Imax);
        RES(i, :) = [1 x1 y1 x2 y2 sqrt((x2-x1)^2 + (y2-y1)^2)/imfinfo(['input/' FILENAMEcontainingResolutionInfo]).XResolution -atand((y2-y1)/(x2-x1))];
    end
    ...
    for i = CC0.NumObjects+1:CC0.NumObjects+CC1.NumObjects
        ...
        y1 = y(Imin);
        y2 = y(Imax);
        RES(i, :) = [2 x1 y1 x2 y2 sqrt((x2-x1)^2 + (y2-y1)^2)/imfinfo(['input/' FILENAMEcontainingResolutionInfo]).XResolution -atand((y2-y1)/(x2-x1))];
    end
    ...
    writetable(array2table(RES, 'VariableNames', {'ROI', 'x1', 'y1', 'x2', 'y2', 'Length', 'Angle'}), 'output/results.xls');
end

% Creates histogram of calculated lengths
RES = readmatrix('output/result.xls');
